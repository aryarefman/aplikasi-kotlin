...
data class HasilBelahKetupat(
    val diagonal1: Double,
    val diagonal2: Double,
    val luas: Double
)

enum class RhombusUnit(val symbol: String, val displayName: String, val toMeter: Double) {
...
object RhombusCalculator {
    fun calculateRhombusProperties(d1: String, d2: String): Pair<HasilBelahKetupat?, ValidationResult> {
        return try {
            when {
                d1.trim().isEmpty() || d2.trim().isEmpty() -> null to ValidationResult.Error("Diagonal tidak boleh kosong")
                    ...
                    when {
                        ...
                        diagonal1 < 0.0001 || diagonal2 < 0.0001 -> null to ValidationResult.Error("Nilai terlalu kecil (minimal 0.0001)")
                            ...
                            HasilBelahKetupat(diagonal1, diagonal2, luas) to ValidationResult.Success
                        }
                    }
                }
            }
    ...
    fun convertValue(valueInBaseUnit: Double, fromUnit: RhombusUnit, toUnit: RhombusUnit): Double {
...
fun RhombusIcon(
    ...
    Canvas(
        modifier = modifier
            .size(160.dp)
            .semantics {
                contentDescription = "Visualisasi belah ketupat animasi"
                role = Role.Image
            }
    ) {
        val strokePx = 8.dp.toPx()
        val width = size.width * 0.6f * animatedScale
        val height = size.height * 0.6f * animatedScale

        val p1 = Offset(center.x, center.y - height / 2)
        val p2 = Offset(center.x + width / 2, center.y)
        val p3 = Offset(center.x, center.y + height / 2)
        val p4 = Offset(center.x - width / 2, center.y)
...
@Composable
fun RhombusAreaCalculatorScreen(navController: NavController) {
    ...
    var hasil by remember { mutableStateOf<HasilBelahKetupat?>(null) }
    var errorMessage by remember { mutableStateOf<String?>(null) }
    var selectedUnit by rememberSaveable { mutableStateOf(RhombusUnit.CM) }
    var isCalculating by remember { mutableStateOf(false) }
    var showUnitPicker by remember { mutableStateOf(false) }
    var previousUnit by remember { mutableStateOf(RhombusUnit.CM) }
    ...
    fun updateDisplayedValues() {
        diagonal1InBaseUnit?.let { d1 ->
            val convertedD1 = RhombusCalculator.convertValue(d1, RhombusUnit.M, selectedUnit)
            diagonal1 = RhombusCalculator.formatNumber(convertedD1)
        } ?: run { diagonal1 = "" }

        diagonal2InBaseUnit?.let { d2 ->
            val convertedD2 = RhombusCalculator.convertValue(d2, RhombusUnit.M, selectedUnit)
            diagonal2 = RhombusCalculator.formatNumber(convertedD2)
        } ?: run { diagonal2 = "" }
    }
    ...
    val calculateArea = remember {
        {
            ...
            val (result, validation) = RhombusCalculator.calculateRhombusProperties(diagonal1, diagonal2)
    ...
    Scaffold(
        ...
        Column(
            modifier = Modifier
                ...
                .semantics {
                    contentDescription = "Layar kalkulator luas belah ketupat"
                },
            verticalArrangement = Arrangement.spacedBy(24.dp),
            horizontalAlignment = Alignment.CenterHorizontally
        ) {
            RhombusIcon(isCalculating = isCalculating)

            Text(
                text = "Masukkan panjang kedua diagonal untuk menghitung luas belah ketupat.",
                ...
                Column(
                    ...
                    OutlinedTextField(
                        ...
                        modifier = Modifier
                            ...
                            .semantics {
                                contentDescription = "Input diagonal 1 belah ketupat dalam satuan ${selectedUnit.displayName}"
                            },
                        singleLine = true,
                    ...
                    OutlinedTextField(
                        ...
                        modifier = Modifier
                            .fillMaxWidth()
                            .semantics {
                                contentDescription = "Input diagonal 2 belah ketupat dalam satuan ${selectedUnit.displayName}"
                            },
                        singleLine = true,
                        isError = errorMessage != null,
                ...
                hasil?.let { result ->
                    Card(
                        modifier = Modifier
                            .fillMaxWidth()
                            .semantics {
                                contentDescription = "Hasil perhitungan luas belah ketupat"
                            },
                        colors = CardDefaults.cardColors(
                        ...
                        Column(
                                ...
                                TextButton(
                                    onClick = {
                                        val resultText = buildString {
                                            appendLine("=== HASIL PERHITUNGAN BELAH KETUPAT ===")
                                            appendLine("Diagonal 1: ${RhombusCalculator.formatNumber(result.diagonal1)} ${selectedUnit.symbol}")
                                            appendLine("Diagonal 2: ${RhombusCalculator.formatNumber(result.diagonal2)} ${selectedUnit.symbol}")
                                            appendLine("Luas: ${RhombusCalculator.formatNumber(result.luas)} ${selectedUnit.symbol}²")
                                        }
                            ...
                            ResultSection(
                                ...
                                Text("Diagonal 1 (d₁) = ${RhombusCalculator.formatNumber(result.diagonal1)} ${selectedUnit.symbol}")
                                Text("Diagonal 2 (d₂) = ${RhombusCalculator.formatNumber(result.diagonal2)} ${selectedUnit.symbol}")
                            }
                            ...
                            ResultSection(
                                ...
                                Text("Luas = ½ × d₁ × d₂ = ½ × ${RhombusCalculator.formatNumber(result.diagonal1)} × ${RhombusCalculator.formatNumber(result.diagonal2)}")
                            }
                            ...
                            ResultSection(
                                ...
                                SelectionContainer {
                                    FinalResult(
                                        label = "Luas",
                                        value = RhombusCalculator.formatNumber(result.luas),
    ...
    if (showUnitPicker) {
        AlertDialog(
            ...
            text = {
                Column {
                    RhombusUnit.values().forEach { unit ->
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                                .clickable {
...
@Composable
private fun FormulaCard(formula: String, color: Color = Color(0xFF0097A7)) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(
